...
use BlackCat\Database\Packages\EntityExternalIds\Repository\EntityExternalIdRepositoryInterface;
...
class EntityExternalIdRepository implements EntityExternalIdRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'entity_table', 'entity_pk', 'source' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\EntityExternalIds\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'external_id' ]);
          return count($rows);
        }
    ...
    public function getByEntityTableAndEntityPkAndSource(string $entityTable, string $entityPk, string $source, bool $asDto = false): array|\BlackCat\Database\Packages\EntityExternalIds\Dto\EntityExternalIdDto|null {
        return $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk, 'source' => $source ], $asDto);
    }
    ...
    public function getIdByEntityTableAndEntityPkAndSource(string $entityTable, string $entityPk, string $source) {
        $row = $this->getByEntityTableAndEntityPkAndSource($entityTable, $entityPk, $source, false);
