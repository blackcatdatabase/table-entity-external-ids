...
namespace BlackCat\Database\Packages\EntityExternalIds\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EntityExternalIds\Definitions;
use BlackCat\Database\Packages\EntityExternalIds\Criteria;
use BlackCat\Database\Packages\EntityExternalIds\Dto\EntityExternalIdDto as Dto;
use BlackCat\Database\Packages\EntityExternalIds\Mapper\EntityExternalIdDtoMapper as RowMapper;
...
final class EntityExternalIdRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EntityExternalIds\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $keys = [ 'entity_table', 'entity_pk', 'source' ];
        if (!$keys) {
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'entity_table', 'entity_pk', 'source' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EntityExternalIds\Definitions::table());
            return $bulk->upsertMany($rows, [ 'entity_table', 'entity_pk', 'source' ], [ 'external_id' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EntityExternalIds\Dto\EntityExternalIdDto|null */
    public function getByEntityTableAndEntityPkAndSource(string $entityTable, string $entityPk, string $source, bool $asDto = false): array|\BlackCat\Database\Packages\EntityExternalIds\Dto\EntityExternalIdDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk, 'source' => $source ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EntityExternalIds\Mapper\EntityExternalIdDtoMapper::fromRow($row);
    }
    public function existsByEntityTableAndEntityPkAndSource(string $entityTable, string $entityPk, string $source): bool {
        $where = 't.' . Ident::q($this->db, 'entity_table') . ' = :uniq_entity_table' . ' AND ' . 't.' . Ident::q($this->db, 'entity_pk') . ' = :uniq_entity_pk' . ' AND ' . 't.' . Ident::q($this->db, 'source') . ' = :uniq_source';
        return $this->exists($where, [ 'uniq_entity_table' => $entityTable, 'uniq_entity_pk' => $entityPk, 'uniq_source' => $source ]);
    }
    /** @return int|string|null */
    public function getIdByEntityTableAndEntityPkAndSource(string $entityTable, string $entityPk, string $source) {
        $row = $this->getByEntityTableAndEntityPkAndSource($entityTable, $entityPk, $source);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EntityExternalIds\Dto\EntityExternalIdDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EntityExternalIds\Dto\EntityExternalIdDto|null {
