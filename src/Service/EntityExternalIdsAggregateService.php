...
namespace BlackCat\Database\Packages\EntityExternalIds\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\EntityExternalIds\Dto\EntityExternalIdDto;
use BlackCat\Database\Packages\EntityExternalIds\Mapper\EntityExternalIdDtoMapper;
 ...
 */
final class EntityExternalIdsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EntityExternalIdRepository $entityExternalIdRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'entity_external_ids' => $this->entityExternalIdRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\EntityExternalIds\ModuleException("Record {$idStr} not found for locking.");
            }
